...
fn construct_version() {
    ...
    let block_ver_with_tx_index = 
        StateDBVersion::from_tx_index([1u8;32].into(), 100u32);
...
fn construct_state_db_txn_from_version() {
    ...
    let version = 
        StateDBVersion::from_block_hash(H256::zero());
    assert!(store.state_at(version).is_err());

    // This case will always be passed, for the db is empty.
    let version = 
        StateDBVersion::from_tx_index(H256::zero(), 5u32);
    assert!(store.state_at(version).is_err());
}
...
fn insert_and_get() {
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 1u64, 1u32);
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 1u64, 2u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[1, 1]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 1u64, 4u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[1, 1]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 1u64, 5u32);
    assert_eq!(
        vec![4, 4, 4].into_boxed_slice(), 
        state_db_txn.get("1", &[2]).unwrap()
    );
...
fn insert_and_get_cross_block() {
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 2u64, 5u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[1, 1]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 3u64, 1u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[1, 1]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 2u64, 4u32);
    assert!(state_db_txn.get("1", &[2]).is_none());

    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 2u64, 6u32);
    assert_eq!(
        vec![0, 2, 2].into_boxed_slice(), 
        state_db_txn.get("1", &[2]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 3u64, 2u32);
    assert_eq!(
        vec![3, 3, 3].into_boxed_slice(), 
        state_db_txn.get("1", &[2]).unwrap()
    );
...
fn insert_keys_with_the_same_version() {
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 3u64, 6u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[1, 1]).unwrap()
    );
...
fn delete() {
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 2u64, 2u32);
    assert_eq!(
        vec![1, 1, 1].into_boxed_slice(), 
        state_db_txn.get("1", &[2]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 2u64, 3u32);
    assert_eq!(
        vec![2, 2, 2].into_boxed_slice(), 
        state_db_txn.get("1", &[2]).unwrap()
    );
    ...
    let state_db_txn = 
        get_state_db_txn_from_tx_index(&store, 4u64, 1u32);
    state_db_txn

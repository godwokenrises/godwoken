...
impl Worker {
  ...
  fn run_spec(&self, spec: &dyn Spec, retried: usize) {
    ...
    let result = panic::catch_unwind(panic::AssertUnwindSafe(|| {
        //TODO: spec.run(&mut nodes);
        spec.run()
    }));

    // error handles
    let spec_error = result.err();
    //TODO: let panicked_error = nodes_panicked(&node_log_paths);
    let panicked_error = false;
    if (panicked_error || spec_error.is_some()) && retried < spec.setup().retry_failed {
        log::error!("{} failed at {} attempt, retry...", spec.name(), retried);
  ...
  pub fn shutdown(&mut self) {
      ...
      for w in &self.workers {
          if let Err(err) = w.0.send(Command::Shutdown) {
              log::info!("shutdown worker failed, error: {}", err);
          }
      }
